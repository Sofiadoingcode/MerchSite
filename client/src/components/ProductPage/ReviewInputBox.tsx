    ...
    const [mutateFunction, {loading, error, data}] = useMutation(GqlCreateReview, {
            refetchQueries: [
                GetAllProducts,
                {
                    ...
                    variables: {reviewsByProductId: product.id}
                }
            ]
        }
    )
                            ...
                            <form onSubmit={handleSubmit}>
                                <CardContent style={{display: 'flex'}}>
                                    <label className="review_label">Title:
                                        <input className="review_input" required value={review.title}
                                               onChange={(evt) => setReview({...review, title: evt.target.value})}/>
                                    </label>
                                    <Rating id="review_input_box_rating" size="large" onChange={(evt, newValue) => {
                                        if (newValue != null) setReview({...review, rating: newValue})
                                    }}/>
                                </CardContent>
                                <CardContent>
                                    <Typography typography={'h6'}>Comment: </Typography>
                                    <textarea required maxLength={500} aria-required className="review_input"
                                              id="review_input_comment" placeholder={"Write your comment here"}
                                              value={review.text}
